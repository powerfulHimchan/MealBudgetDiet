...
test.describe("Google Play 공개 표면", () => {
  test("로그인 없이 개인정보처리방침과 계정 삭제 안내를 조회한다", async ({ page }) => {
    ...
    await expect(page).toHaveTitle("개인정보처리방침 | MealBudgetDiet");
    ...
    await expect(page).toHaveTitle("계정 삭제 안내 | MealBudgetDiet");
    await expect(page.getByRole("heading", { level: 1, name: "MealBudgetDiet 계정 삭제" })).toBeVisible();
